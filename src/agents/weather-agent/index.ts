...
const systemPrompt = readFileSync(join(__dirname, 'weather_agent.prompt'), 'utf-8');
...
class WeatherAgentExecutor implements AgentExecutor {
  async execute(
    ...
    if (!historyForAnthropic.find(m => m.messageId === userMessage.messageId)) {
    ...
    try {
      ...
      if (requestContext.isCancelled()) {
        console.log(`[WeatherAgentExecutor] Request cancelled for task: ${taskId}`);
          ...
          if (content.type === 'tool_use') {
            try {
              ...
              toolResults.push({
                type: 'tool_result',
                tool_use_id: content.id,
                content: JSON.stringify({ error: `Error executing tool: ${error}` }, null, 2)
              });
            }
          }
        }
...
async function main() {
  ...
  expressApp.listen(PORT, () => {
    console.log(`[WeatherAgent] Server using new framework started on http://localhost:${PORT}`);
    console.log(`[WeatherAgent] Agent Card: http://localhost:${PORT}/.well-known/agent.json`);
